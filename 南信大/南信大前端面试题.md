...
- 存储时间：
  - localStorage存储的数据是永久性的，直到用户删除
  - sessionStorage存储的数据与页面会话时间有关，一旦窗口或标签页被关闭，则存储的数据也会消失
- 访问权限：
  - localStorage只要在相同的协议、相同的主机名、相同的端口下，就能读取/修改到同一份数据
  - sessionStorage要求更加苛刻，需要在同一个窗口下才能读取/修改到同一份数据
...
cookie是服务器发送到用户浏览器并保存在本地的一小块数据，他会在浏览器下次向同一服务器再发起请求时被携带并发送到服务器上。通常，它用于告知服务端两个请求是否来自同一浏览器，如保持用户的登录状态。
...
- 存储大小：web storage存储大小可达5M，cookie则为4k左右
- 时间限制：cookie可以设置失效时间
- 性能问题：cookie在浏览器的每次请求中都会携带，web storage 存储在浏览器
...
```js
var map_Gaode = null
map_Gaode = new AMap.Map('map_Gaode', {
  resizeEnable: true,
  zoom: 7,
  center: [109.6899999, 19.0000001]
})
```
...
CSS实现动画主要有两种方法：`transition`和`animation`属性。

`transition`

**transition`实例**

先看一个`transition`实现长度拉伸的例子：
...
CSS `animation`就是为了上面这些问题而提出的。`animation`是一个非常成熟的可以用来展现动画效果的属性。
...
```js
      ...
      this.chartTimer = setInterval(() => {
        ...
        index++;
        if (index >= this.precipitationList.length) {
          index = 0;
        }
      }, 1000);
```
...
### uniapp中使用map组件进行定位并绘制出中心点和范围圆圈
...
下载插件源码，并将`u-charts.js`所在文件夹导入项目的components文件下。

```html
<template>
    <view>
        <view class="qiun-bg-white qiun-title-bar qiun-common-mt">
            <view class="qiun-title-dot-light">7天产量折线图</view>
...
### uniapp如何调用摄像头拍摄视频然后加载到Video里面播放
...
   // 退出登录
   change(){
    // 这里只是改变了按钮文字内容，真正退出需要清除token，回到首页。
    this.logState = '已登录';
   }
  }
</script>
